<template>
  <div class="message-input" v-if="activeRoom">
    ...
    <div sstyle="width: 15%;">
      <a-switch  @change='onChange' style="position: absolute;right: 18px;top: 7px;" size="large"  checkedChildren="AUTO" unCheckedChildren="OFF" defaultChecked/>
    </div>
  </div>
</template>
...
export default class GenalInput extends Vue {
  ...
  preSendMessage() {
    ...
    if (this.text.length > 220) {
      this.$message.error('Message too long!');
      return;
    }
    ...
    // }
    this.$emit('insertTextToUser', this.text);
