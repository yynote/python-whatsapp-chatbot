  ...
  <div class="message">
    <div class="message-header">
      <div class="message-header-box">
        <span class="message-header-text">{{selectUser.userId }}</span>
    ...
    <div class="message-main" :style="{ opacity: messageOpacity }">
      <div class="message-content">
        <transition name="noData">
          <!-- <div class="message-content-noData" v-if="isNoData">No More Use~</div> -->
          <!-- <div class="message-content-noData" v-if="true">No More Use~</div> -->
      
        </transition>
        <template v-for="item in selectUser.messages">
          <div class="message-content-message" :key="item.userId + item.time" :class="{ 'text-right': (item.userId === activeRoom.userId || item.userId === 'bot')}">
            <!-- <genal-avatar :data="item"></genal-avatar> -->
            <div>
              <!-- <a clasdws="message-content-text" v-if="_isUrl(item.content)" :href="item.content" target="_blank">{{ item.content }}</a> -->
              <div class="message-content-text" v-text="item.content" :style="{'background-color':  item.userId === activeRoom.userId ?  'rgba(0, 80, 0, 0.4)' : (item.userId === 'bot' ? 'rgba(70, 0, 0, 0.4)' : 'rgba(0, 0, 0, 0.4)')}"></div>
              ...
              <div v-text="item.userId === activeRoom.userId ?('Answered by ' + item.userId + ' ' + item.time):(item.userId === 'bot' ? ('Answered by Bot ' + item.time) : ('From ' + item.userId +' '+ item.time))" style="text-align: center; font-size: small;"></div>
            </div>
          </div>
        </template>
      </div>
    </div>
    <genal-input @insertTextToUser="insertTextToUser"></genal-input>
  </div>
</template>
...
export default class GenalMessage extends Vue {
  ...
  @chatModule.Getter('selectUser') selectUser: Friend;
  ...
  get chatName() {
    console.log('this', this.selectUser["userId"])
    // let temp = this.selectUser.userId
    return "johnff"
  ...
  @Watch('activeRoom')
  changeActiveRoom() {
    console.log('data', this.activeRoom);     
   ...
   * 在分页信息的基础上来了新消息
   */
  insertTextToUser(value: string){
    const date = new Date();
    const time = date.toLocaleTimeString([], { hour: 'numeric', minute: 'numeric', hour12: true });
    const dateString = date.toLocaleDateString();

    const formattedDate = `${time} ${dateString}`;
    this.selectUser.messages.push({userId: 'Nick', friendId: this.selectUser.userId, content: value, messageType: "text", time :formattedDate, view: 'off'})
    this.socket.emit('saveNewRecord', {from: 'Nick', to: this.selectUser.userId, message: value, messageType: "text", time :formattedDate, view: 'on'})
  }
  addMessage() {
