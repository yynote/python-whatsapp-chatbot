...
const mutations: MutationTree<ChatState> = {
  ...
  [SET_SELECT_USERS](state, payload: Friend & Group) {
    console.log('setselect', payload)
    state.selectUser = payload;
  },
  [SET_USER_INFO](state, payload: Friend & Group) {
  ...
  [INSERT_USER_INFO](state, payload: Friend & Group) {
    state.userInfo.map(index => {
      if(index.userId == payload.from){
        index.unreadMsg++;
        index.messages.push({
      ...
      console.log('userID', index.messages)

      }

    })
    state.activeRoom?.messages?.push(payload)
  },
  ...
  // 给某个聊天组清空未读消息
  [CHANGE_UNREAD_MSG](state, payload: any) {
    console.log('change_uread', payload)
    state.userInfo.map(index => {
      if(index.userId == payload.userId){
      Vue.set(index, 'unreadMsg', 0);
      }
    })
  },
};
